...
public class RequestLoanPage extends BasePage {
    ...
    private By applyButton = By.xpath("//input[@type='button' and @value='Apply Now']");
    ...
    public void selectFromAccountByIndex(int index) {
        WebElement element = wait.until(ExpectedConditions.presenceOfElementLocated(fromAccount));
    ...
    public void requestLoan(String loanAmount, String payment, int accountIndex) {
    ...
    public boolean isLoanApproved() {
        try {
            wait.until(ExpectedConditions.visibilityOfElementLocated(approvedMessage));
            return true;
        } catch (Exception e) {
            return false;
        }
    ...
    public boolean hasErrorMessage(String text) {
        try {
            return wait.until(driver -> driver.findElements(By.cssSelector("p.error"))
                            .stream().anyMatch(element ->
                                    element.isDisplayed() && element.getText().contains(text)));
        } catch (Exception e) {
            return false;
        }
    }
    public boolean hasInternalError() {
        return hasErrorMessage("An internal error has occurred and has been logged.");
    }
    public boolean hasLargeAmountError() {
        return hasErrorMessage("We cannot grant a loan in that amount with your available funds.");
    }
    public boolean hasInsufficientFundsError() {
        return hasErrorMessage("You do not have sufficient funds for the given down payment.");
    }
}