...
public class TransferFundsPage {
    ...
    private By transferButton = By.xpath("//input[@type='submit' and @value='Transfer']");
    private By successHeading = By.xpath("//h1[contains(text(),'Transfer Complete')]");
    private By errorHeading = By.xpath("//h1[contains(text(),'Error!')]");
    ...
    public void selectFromAccountByIndex(int index) {
        WebElement element = wait.until(ExpectedConditions.presenceOfElementLocated(fromAccountDropdown));
        Select select = new Select(element);
        wait.until(driver -> select.getOptions().size() > index);
        select.selectByIndex(index);
    }
    public void selectToAccountByIndex(int index) {
        WebElement element = wait.until(ExpectedConditions.presenceOfElementLocated(toAccountDropdown));
        Select select = new Select(element);
        wait.until(driver -> select.getOptions().size() > index);
        select.selectByIndex(index);
    }
    ...
    public boolean isErrorDisplayed() {
        try {
            wait.until(ExpectedConditions.visibilityOfElementLocated(errorHeading));
            return true;
        } catch (Exception e) {
            return false;
        }
